...
const groups=selection.groups.map(g=>{
 ...
 for(const file of g.members){
  ...
  people.set(file,{...publicFields(p),id:file,kind:'person',school:g.school,group:g.id,domain:g.domain,
   aliases:p.meta.aliases||[],role:({professor:'教研人员',student:'学生 / 研究成员',founder:'创业者',researcher:'研究人员'}[p.meta.role_type]||'教研人员'),
...
// Cross-platform directory relations are separate from primary display placement.
for(const [file,members] of Object.entries(selection.platformMembers)){
 const g=groups.find(g=>g.file===file),a=article(g.path);
 for(const id of members){
  if(!people.has(id)||!a.text.includes(`${id}.md`))throw new Error(`Missing platform evidence: ${id}/${file}`);
  edges.push({source:g.id,target:id,label:'平台目录收录 · 不推定全职或行政隶属',type:'collab',evidence:[g.path]});
 }
}
...
const data={updated:'2026-09-22',title:'北京大学人才地图',domains:[['perception','感知与空间智能'],['learning','具身模型与学习'],['control','控制与自主系统'],['hardware','本体与操作'],['foundation','基础模型与算力'],['medical','医疗与脑机交叉'],['speech','语音与生成AI'],['energy','能源与材料'],['science','AI科学实验']],schools,groups,people:[...people.values()],companies:[...companies.values()],edges,affiliations};
fs.writeFileSync(path.join(__dirname,'../data.js'),`// Generated by source/build.cjs; edit the selection, then rebuild.\nwindow.PKU_MAP = ${JSON.stringify(data,null,2).replace(/</g,'\\u003c')};\n`);
console.log(`PKU map: ${schools.length} schools, ${groups.length} reading groups, ${people.size} unique people, ${companies.size} companies, ${edges.length} relationships.`);
