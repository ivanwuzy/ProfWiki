(() => {
 'use strict';
 const data=window.PKU_MAP, $=id=>document.getElementById(id);
 ...
 function displayGroups(s){
  ...
  if(affiliates.length)list.push({id:`cross-${s.id}`,school:s.id,name:'跨院关联与历史任职',parent:'同一人物 · 分阶段记录',members:affiliates.map(a=>a.person),affiliates,synthetic:true});
  return list;
 }
 ...
 function render(){
  ...
  $('view-title').textContent=school?schoolLabel(school):'院系与研究平台';
 ...
 function selectEntity(id,jump=false){
  ...
  $('detail-kind').textContent=item.kind==='person'?'人物':item.kind==='company'?'公司 / 创业项目':schools.has(id)?'院系 / 研究入口':'研究团队 / 平台';
  ...
  if(people.has(id)){
   if(item.affiliation)$('detail-body').append(el('p','scope-note',item.affiliation));
