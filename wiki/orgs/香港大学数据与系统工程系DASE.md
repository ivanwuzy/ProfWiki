---
type: org
name: 香港大学数据与系统工程系DASE
aliases: [HKU Department of Data and Systems Engineering, HKU DASE]
org_type: department
tier: 1
parent: 香港大学
domains: [ai, robotics, micro-robotics, medical-robotics, systems-engineering]
key_people: [刘俊]
related_companies: []
watch_tier: T1
priority: A
last_verified: 2026-09-07
confidence: verified_primary
tags: [org, hku, frontier-talent]
---

# 香港大学数据与系统工程系DASE

> Sources: HKU DASE官网（https://www.dase.hku.hk/）；AIR Lab官网（https://airlab.hku.hk/）
> Raw: [香港大学工程学院院系与重点人物调研报告](../../raw/2026-09-07-香港大学工程学院院系与重点人物调研报告.md)

## 一句话判断

香港大学数据与系统工程系，成立于1973年，刘俊主持AIR Lab聚焦微纳米机器人与医疗AI，QS 2026数据科学全球第18位，系统工程与AI智能系统结合。

## 基本信息

| 字段 | 内容 |
|---|---|
| 类型 | 院系 / department |
| 英文全称 | Department of Data and Systems Engineering |
| 所属单位 | 香港大学 |
| 成立时间 | 1973年 |
| QS排名2026 | 数据科学全球第18位 |
| 方向 | 系统分析、AI、智能系统、微纳米机器人、医疗AI、运筹学、优化 |
| 官网 | https://www.dase.hku.hk/ |

## 重点人物

| 人物 | 角色 | 方向 | 信号 |
|---|---|---|---|
| [刘俊](../people/刘俊_港大DASE.md) | 副教授、AIR Lab负责人 | 微纳米机器人、医疗AI、智能传感器 | 15年+微纳米与医疗机器人经验，Nature Communications |

## 下属实验室/课题组

本库目前已给「香港大学数据与系统工程系DASE」建立以下 tier: 2 实验室/研究组页面：

| 实验室/研究组 | 负责人 | 重点方向 | 页面链接 |
|---|---|---|---|
| AIR Lab | [刘俊](../people/刘俊_港大DASE.md) | 微纳米机器人、医疗机器人、单细胞操作、IVF应用 | [港大AIR Lab](港大AIR_Lab.md) |

## 核心研究方向

### 1. 微纳米机器人（Micro/Nano Robotics）
- **刘俊AIR Lab**：单细胞操控、药物筛选、细胞间通讯表征
- 体外受精生殖细胞处理
- 微纳米机器人操控技术

### 2. 医疗AI与制造AI
- **刘俊AIR Lab**：医疗图像分析
- 微创医疗器械
- AI在医疗保健中的应用
- 先进制造中的AI应用

### 3. 智能传感器与执行器
- 软机器人研究
- 智能材料应用
- 减少辅助传感器的复杂任务执行

### 4. 系统集成
- 传感器、控制系统、AI算法、执行器集成
- 面向医疗保健、建筑、先进制造的机器人和自动化系统

### 5. 系统工程与优化
- 系统分析
- 运筹学
- 数学与数据驱动优化
- 系统仿真与建模
- AI与智能系统

## 学术影响

### 刘俊AIR Lab成果
- **15年+经验**：微纳米与医疗机器人研究
- **顶刊论文**：Nature Communications等
- **职业经历**：曾任香港城市大学机械工程系助理教授（2019年起），2024年晋升副教授后加入HKU

### 学科地位
- **成立时间**：1973年，50年历史
- **QS排名**：数据科学全球第18位（2026）
- **学科定位**：AI、大数据、系统工程前沿

## 关联研究院

- **HKU Musketeers Foundation Institute of Data Science（HKU-IDS）**：DASE关联研究院

## 组织定位

DASE在香港大学工程学院中承担系统工程与AI智能系统研究，同时在机器人方向有独特优势：
- **微纳米机器人**：刘俊AIR Lab，医疗应用，临床转化潜力
- **系统工程**：运筹学、优化、系统建模
- **AI系统**：智能系统设计与集成
- **医疗AI**：医疗图像分析、微创器械

与香港大学计算与数据科学学院CDS（AI算法）、香港大学电气与计算机工程系ECE（硬件）形成AI全栈生态。

## 待验证

- [ ] DASE完整教师名单和研究方向
- [ ] 刘俊AIR Lab团队成员和博士生名单
- [ ] AIR Lab代表性论文和专利
- [ ] 与产业界的具体合作项目
- [ ] DASE与HKU-IDS的组织关系

## 证据

| 来源 | 支持事实 | 日期 |
|---|---|---|
| HKU DASE官网 | DASE基本信息、成立时间、QS排名、研究方向 | 2026-09-07 |
| AIR Lab官网 | 刘俊负责人、实验室研究方向、团队信息 | 2026-09-07 |
| 刘俊个人主页 | 职业经历、研究成果 | 2026-09-07 |
| HKU Knowledge Exchange报道 | 微纳米与医疗机器人开创性进展 | 2026-09-07 |
