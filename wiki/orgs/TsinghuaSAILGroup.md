---
type: org
name: 清华大学 SAIL Group
aliases: ["Tsinghua SAIL Group"]
org_type: group
tier: 2
parent: 清华大学计算机科学与技术系
domains: []
key_people: []
related_companies: []
watch_tier: T2
priority: A
last_verified: 2026-06-11
confidence: verified_primary
tags: [org, tsinghua, frontier-talent]
---

# 清华大学 SAIL Group

> Sources: SAIL Group官网[^source-1]
> Raw: [SAIL Group官网](../../raw/sources/2026-06-11-TsinghuaSAILGroup-SAILGroup官网-da35a2b8.md)

## 一句话判断

Statistical Artificial Intelligence & Learning；机器学习理论、算法与科学/工程应用。

## 基本信息

| 字段 | 内容 |
|---|---|
| 类型 | 实验室/课题组 / group |
| 所属单位 | [清华大学计算机科学与技术系](清华大学计算机科学与技术系.md) |
| 方向 | Statistical Artificial Intelligence & Learning；机器学习理论、算法与科学/工程应用。 |
| 官网 | SAIL Group官网 |

## 重点人物

| 人物 | 角色 | 方向 | 信号 |
|---|---|---|---|
| 待抽取 | 见 raw 快照和官方页面 | PI/学生、论文、代码、研究方向、毕业去向。 | 创建实验室节点；作为朱军等机器学习人才池入口。 |

## 创业/产业化线索

| 公司/项目 | 关联方式 | 相关人物 | 证据 |
|---|---|---|---|
| 待验证 | 创建实验室节点；作为朱军等机器学习人才池入口。 | 待抽取 | [^source-1] |

## 导入字段

PI/学生、论文、代码、研究方向、毕业去向。

## 后续动作

创建实验室节点；作为朱军等机器学习人才池入口。

## 证据

| 来源 | 支持事实 | 日期 |
|---|---|---|
| SAIL Group官网[^source-1] | 支持“Tsinghua SAIL Group”的定位、人员入口或建页动作 | 2026-06-11 |

## 引用来源

[^source-1]: [SAIL Group官网](https://ml.cs.tsinghua.edu.cn/)
