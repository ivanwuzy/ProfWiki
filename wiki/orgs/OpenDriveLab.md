---
type: orgs
name: 香港大学 OpenDriveLab
aliases: ["OpenDriveLab","开源自动驾驶实验室"]
parent: 香港大学计算与数据科学学院
tier: 2
domains: [autonomous-driving, embodied-ai, end-to-end-driving, computer-vision]
key_people: [李弘扬]
last_verified: 2026-09-08
confidence: verified_primary
tags: [orgs, hku, autonomous-driving, tier2]
---

# 香港大学 OpenDriveLab

> Sources: [OpenDriveLab官网](https://opendrivelab.com/)；[GitHub](https://github.com/opendrivelab)；[个人主页](https://lihongyang.info/)；[OpenDriveLab研究动态](https://opendrivelab.com/)；[HKU-MMLab团队介绍](https://mmlab.hk/about-us)
> Updated: 2026-09-08
> Raw: [李弘扬本人主页](../../raw/sources/李弘扬官方个人主页.md)；[香港大学工程学院院系与重点人物调研报告](raw/sources/2026-09-07-香港大学工程学院院系与重点人物调研报告.md)；[OpenDriveLab研究动态](../../raw/sources/香港具身核查-OpenDriveLab研究动态.md)；[HKU-MMLab团队介绍](../../raw/sources/香港具身核查-HKU-MMLab团队介绍.md)

## 一句话判断

HKU计算与数据科学学院下属的开源自动驾驶研究实验室，由李弘扬联合创办（2021年），产出UniAD等CVPR最佳论文，是端到端自动驾驶与具身智能方向的前沿研究平台。

## 基本信息

| 字段 | 内容 |
|---|---|
| 名称 | OpenDriveLab |
| 类型 | tier 2 实验室 |
| 所属单位 | [香港大学计算与数据科学学院](香港大学计算与数据科学学院CDS.md) |
| 负责人 | [李弘扬](../people/李弘扬_港大CDS.md) |
| 创建时间 | 2021年 |
| 官网 | https://opendrivelab.com/ |

## 研究方向

### 端到端自动驾驶
- 规划导向的自动驾驶系统
- 感知-预测-规划联合优化
- 多任务学习框架
- 驾驶策略学习

### 具身智能系统
- 具身智能体设计
- 空间智能
- 机器人操作与决策
- 世界模型

### 计算机视觉
- 3D场景理解
- 多模态感知
- 视觉预测
- 动态环境建模

## 关键成果

### CVPR 2023最佳论文

**UniAD: Planning-oriented Autonomous Driving**：
- IEEE CVPR 2023最佳论文奖
- 端到端自动驾驶系统
- 规划导向的统一框架
- GitHub开源项目：https://github.com/opendrivelab/uniad

### 研究产出
- 持续在CVPR/ICCV/NeurIPS等顶会发表论文
- 多个开源项目与数据集
- 活跃的学术社区

## 团队成员

### 核心成员
- [李弘扬](../people/李弘扬_港大CDS.md)：联合创始人、HKU助理教授
- 其他联合创始人待补充
- 研究团队成员待补充

### 合作机构
- Shanghai AI Lab（上海AI实验室）
- 其他合作机构待补充

## 平台与资源

### 开源项目
- UniAD：端到端自动驾驶系统
- 其他项目待补充

### 社区活动
- GitHub组织活跃维护
- 学术交流与合作
- 开源社区建设

## 产业影响

### 技术转化
- 端到端自动驾驶技术向产业输出
- 与自动驾驶公司潜在合作
- 开源生态建设

### 影响力
- CVPR最佳论文标志性成果
- 自动驾驶研究前沿位置
- 国际学术影响力

## 待验证

- [ ] 其他联合创始人名单
- [ ] 完整团队成员列表
- [ ] 当前在读博士生/硕士生名单
- [ ] 所有开源项目清单
- [ ] 具体产业合作伙伴
- [ ] 实验室物理位置与设施
- [ ] 资金来源与规模
- [ ] 是否有衍生创业项目

## 证据

| 来源 | 支持事实 | 日期 |
|---|---|---|
| OpenDriveLab官网 | 实验室介绍、研究方向 | 2026-09-07 |
| IEEE CVPR 2023 | UniAD最佳论文奖 | 2023-06 |
| GitHub opendrivelab | 开源项目维护 | 2026-09-07 |
| 李弘扬个人主页 | 2021年联合创办 | 2026-09-07 |

## 本轮运行核验（2026-09-08）

官网当前列“Archon & OpenDriveLab at RSS 2026”、CoRL 2026及近期具身研究内容，支持持续活跃。HKU-MMLab官网把负责人[李弘扬](../people/李弘扬_港大CDS.md)列为成员，形成跨团队人才网络；不由此改变本页已确认的CDS导航父节点。[源策未来](../companies/源策未来ArchonRobotics.md)沿用已核验公司页，不重复建实体。
