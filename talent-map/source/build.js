...
// A shared path serves both the quiet base map and the selected relationship.
const groupsByKey=new Map(cards.flatMap(c=>c.groups).map(g=>[g.key,g]));
const cardsByDept=new Map(cards.map(c=>[c.dept,c]));
function relationPoints(edge){
 const p=P[edge.source];
 if(edge.kind==='company'){
  const c=D.instances.find(c=>c.name+'@'+c.person===edge.target);
  if(p.group.startsWith('HEAD')&&p.bubbles.indexOf(c)>0){const rail=p.y-p.h/2-.18;
   return [[p.x,p.y-p.h/2],[p.x,rail],[c.x,rail],[c.x,c.y-c.d/2]];}
  return [[p.x+p.w/2,p.y],[c.x-c.d/2,c.y]];
 }
 const q=P[edge.target],g=groupsByKey.get(p.group),h=groupsByKey.get(q.group);
 if(p.group===q.group&&Math.abs(p.y-q.y)<.1){
  const direction=q.x>=p.x?1:-1;
  return [[p.x+direction*p.w/2,p.y-.12],[q.x-direction*q.w/2,q.y-.12]];
 }
 const exit=p.x-p.w/2+.18,startY=p.y-p.h/2-.11,endY=q.y-q.h/2-.17;
 const sourceRail=g?g.x+.12:cardsByDept.get(p.dept).x+.12;
 const targetRail=h?h.x+.12:cardsByDept.get(q.dept).x+.12;
 if(p.group===q.group)return [[exit,p.y-p.h/2],[exit,startY],[sourceRail,startY],
  [sourceRail,endY],[q.x,endY],[q.x,q.y-q.h/2]];
 // Cross-group links leave through the row gutters. They are painted below
 // the cards, so even a long connector cannot obscure a person's name.
 const bendY=Math.min(startY,endY)-.26;
 return [[exit,p.y-p.h/2],[exit,startY],[sourceRail,startY],[sourceRail,bendY],
  [targetRail,bendY],[targetRail,endY],[q.x,endY],[q.x,q.y-q.h/2]];
}
svg.push('__RELATION_LAYER__');

// --- person nodes: rounded cards, status carried by an inset accent bar
for(const p of D.people){
 svg.push(`<g class="map-node" data-node-id="${escAttr(p.name)}">`);
...
for(const c of D.instances){const fill=c.pending?C.coPendFill:c.historical?C.sunken:c.core?C.coCoreFill:C.coPlainFill,stroke=c.pending?C.pending:c.historical?C.lineStrong:c.core?C.coCoreFill:C.founder,tc=c.core&&!c.pending&&!c.historical?C.coCoreInk:c.pending?C.coPendInk:C.coPlainInk;
 svg.push(`<g class="map-node" data-node-id="${escAttr(c.name+'@'+c.person)}">`);
...
const items=geometry.filter(g=>['person','context','company','lab','department','platform'].includes(g.type)).map(g=>{
 ...
 if(g.type==='person'){const p=P[name];info=[p.dept,p.identity,p.detail,...p.notes,...(p.extraInfo||[])];urls=p.urls||[];links=p.bubbles.map(c=>({label:`${c.name}｜${c.role}${c.historical?'（历史参与）':''}`,target:c.name+'@'+c.person}));}
 ...
 else if(g.type==='lab'){const m=D.labMeta[name]; info=m?[m.cn,m.scope,m.direction,...(m.explanatory?['中文为译名或说明名']:[]),...(m.extraInfo||[])]:[];urls=m?.urls||[];}
...
const relations=compileRelations(D,items);
for(const edge of relations){
 edge.points=relationPoints(edge).map(point=>point.map(n=>n*px));
 edge.path=edge.points.map(([x,y],i)=>`${i?'L':'M'}${x},${y}`).join(' ');
}
const edgeColor=e=>C[e.type==='historical'||e.type==='collab'?'none':e.type];
const edgePath=(e,active)=>`<path ${active?'class="relation-path d2" hidden':'class="base-relation '+(e.kind==='company'?'d2':'d3')+'"'} data-relation-id="${escAttr(e.id)}" d="${e.path}" fill="none" stroke="${edgeColor(e)}" stroke-width="${active?2.4:1.5}"${['pending','historical','collab'].includes(e.type)?' stroke-dasharray="5 5"':''}${e.type==='mentor'?` marker-end="url(#${active?'active-':'arrow-'}mentor)"`:''}${active?' vector-effect="non-scaling-stroke"':''}><title>${esc(e.label)}${e.pending?'（待核）':''}${e.historical?'（历史参与）':''}</title></path>`;
const activeMarker=`<defs><marker id="active-mentor" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="${C.mentor}"/></marker></defs>`;
const relationLayer=relations.filter(e=>e.kind==='company'||P[e.source].group===P[e.target].group&&!P[e.source].group.startsWith('HEAD')).map(e=>edgePath(e,false)).join('')+
 `<g id="connections" aria-hidden="true">${activeMarker}${relations.map(e=>edgePath(e,true)).join('')}</g>`;
svg=svg.map(part=>part==='__RELATION_LAYER__'?relationLayer:part);
...
const data={width:W*px,height:H*px,stats,items,relations,cards:cards.map(c=>({name:c.dept,kind:'department',isCard:true,info:[D.deptThemes[c.dept]],wikiUrl:wikiUrl('department',c.dept),x:c.x*px,y:c.y*px,w:c.w*px,h:c.h*px}))};
