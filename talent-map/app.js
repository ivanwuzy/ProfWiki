...
function fit(){fitScale=Math.min((view.clientWidth-24)/5184,(view.clientHeight-24)/4536);scale=fitScale;x=(view.clientWidth-5184*scale)/2;y=(view.clientHeight-4536*scale)/2;draw();}
function zoom(factor,cx=view.clientWidth/2,cy=view.clientHeight/2){const next=Math.max(fitScale*.7,Math.min(3,scale*factor));x=cx-(cx-x)*next/scale;y=cy-(cy-y)*next/scale;scale=next;draw();}
function focus(item,card=false){scale=card?Math.min((view.clientWidth-40)/item.w,(view.clientHeight-40)/item.h):Math.min(1.5,view.clientWidth/520);x=view.clientWidth/2-(item.x+item.w/2)*scale;y=view.clientHeight*(card?.5:.42)-(item.y+item.h/2)*scale;const h=$('highlight');Object.assign(h.style,{left:(item.x-8)+'px',top:(item.y-8)+'px',width:(item.w+16)+'px',height:(item.h+16)+'px'});h.hidden=false;draw();if(!card){$('detail-kind').textContent=labels[item.kind];$('detail-name').textContent=item.name;$('detail-info').replaceChildren(...item.info.map(t=>{const p=document.createElement('p');p.textContent=t;return p;}));$('detail').hidden=false;}else $('detail').hidden=true;}
